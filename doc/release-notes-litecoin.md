Timocoin Core version *0.15.1* is now available from:

  <https://download.timocoin.org/timocoin-0.15.1/>
...
A number of changes to the way Timocoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 timos per kB (0.00001 TMC). This is to prepare for dropping the minimum transaction fee to 0.1 timos per kB (0.0001 TMC) in the next release.
