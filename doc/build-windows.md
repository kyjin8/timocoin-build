...
Below are some notes on how to build Timocoin Core for Windows.

The options known to work for building Timocoin Core on Windows are:

* On Linux using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Ubuntu Trusty 14.04 is recommended
and is the platform used to build the Timocoin Core Windows release binaries.
...
for Ubuntu Xenial does not produce working executables for some of the Timocoin Core applications.
...
Note that for WSL the Timocoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/timocoin, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
This means you cannot use a directory that located directly on the host Windows file system to perform the build.

The next three steps are an example of how to acquire the source in an appropriate way.

    cd /usr/src
    sudo git clone https://github.com/timocoin-project/timocoin.git
    sudo chmod -R a+rw timocoin
...
way. This will install to `c:\workspace\timocoin`, for example:

    make install DESTDIR=/mnt/c/workspace/timocoin
...
causes two of the timocoin executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the timocoin code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the timocoin source code).
